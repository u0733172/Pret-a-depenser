...
def main() :
    ...
    @st.cache_data
    def load_prediction(sample, _id, _clf):
        X=sample.iloc[:, :-1]
        score = _clf.predict_proba(X[X.index == int(_id)])[:,1]
        return score
    ...
    st.markdown('***')
    st.markdown("Merci de visionner cette application web. Pour les commentaires clicker sur le lien *Code from [Github](https://github.com/u0733172/Pret-a-depenser.git)* ❤️")
